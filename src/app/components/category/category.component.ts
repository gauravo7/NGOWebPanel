...
export class CategoryComponent implements OnInit {
  ...
  openModel(type: string, categoryId: any) {
    ...
    if (categoryId !== null) {
      this.spinner.show();
      this.categoryService.singleCategory({ _id: categoryId }).subscribe({
        next: (v: any) => {
          this.spinner.hide();
          if (v.success) {
 ...
 uploadImage(event: any) {
  ...
  getAllCategories(event: any) {
    ...
    this.categoryService.allCategories(data).subscribe({
      next: (result) => {
        this.spinner.hide();
        if (result.success) {
  ...
  submit() {
    ...
    data.append('category_image', this.categoryForm.value.category_image ?? '');
      ...
      .then((result) => {
        if (result.isConfirmed) {
          this.categoryService
            .enableDisableCategory({ _id: element._id, isBlocked: newStatus })
            .subscribe({
              next: (v: any) => {
                if (v.success) {
